...
function selectFlow(id) {
  ...
  document.getElementById('int-headers-count').innerHTML = `(${Object.keys(item.req_headers || {}).length})`;
  document.getElementById('int-cookies-count').innerHTML = (item.req_cookies || []).length ? `(${item.req_cookies.length})` : '';
...
function renderInterceptDetail() {
  ...
  el.innerHTML = buildTabHtml(intTab, item, 'request');
  wireAccordions(el);
}
...
function updateScopeSummary(s) {
  ...
  const methodsTxt = (s.methods || []).length === ALL_METHODS.length ? '' : `[${(s.methods || []).join(',')}]`;
