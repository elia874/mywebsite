...
def in_scope(flow: http.HTTPFlow, verbose=False) -> bool:
    req = flow.request
    ...
    # IMPORTANT: an empty methods list means "no method restriction" (matches
    # everything), exactly like an empty domain_include/path_regex does.
    # Previously an empty list meant "match nothing", which silently blocked
    # 100% of traffic if every method chip happened to be unchecked -- with
    # zero indication anything was wrong. That was the real bug.
    if methods and req.method not in methods:
        if verbose:
            print(f"[scope] SKIP {req.method} {host}{req.path} -- method not in scope list {methods}")
        return False

    if excludes and any(x.lower() in host for x in excludes if x):
        if verbose:
            print(f"[scope] SKIP {req.method} {host}{req.path} -- host matches an exclude rule")
        return False
    if includes and not any(x.lower() in host for x in includes if x):
        if verbose:
            print(f"[scope] SKIP {req.method} {host}{req.path} -- host doesn't match any include rule {includes}")
        return False
    ...
    if pattern:
        try:
            if not re.search(pattern, req.path):
                if verbose:
                    print(f"[scope] SKIP {req.method} {host}{req.path} -- path doesn't match regex '{pattern}'")
                return False
        except re.error:
            pass  # bad regex -> don't crash the proxy, just don't filter on it

    if verbose:
        print(f"[scope] MATCH {req.method} {host}{req.path} -- in scope")
    return True
...
def _flow_summary(flow_id, entry):
    ...
    return {
        ...
        "req_headers": req_headers,
        "req_body": body,
        "req_length": len(body.encode("utf-8", errors="replace")) if body else 0,
        "conn": _connection_info(flow),
        "application": _guess_application(req_headers),
        "req_cookies": _parse_request_cookies(req_headers),
        "ts": time.strftime("%H:%M:%S"),
...
class MobileInterceptAddon:
    ...
    async def request(self, flow: http.HTTPFlow):
        ...
        should_freeze = enabled and in_scope(flow, verbose=enabled)
...
def _sanitize_domain(entry):
    e = (entry or "").strip()
    e = re.sub(r"^[a-zA-Z][a-zA-Z0-9+.-]*://", "", e)   # strip any scheme (http://, https://, ...)
    e = e.split("/")[0]                                  # strip path if a full URL was pasted
    e = e.split(":")[0] if e.count(":") == 1 else e      # strip a bare :port, but leave IPv6-ish strings alone
...
def api_settings_set():
    ...
    with _lock:
        ...
        if "domain_include" in payload:
            SETTINGS["domain_include"] = [_sanitize_domain(x) for x in payload["domain_include"] if x and x.strip()]
        if "domain_exclude" in payload:
            SETTINGS["domain_exclude"] = [_sanitize_domain(x) for x in payload["domain_exclude"] if x and x.strip()]
...
<style>
  ...
  * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
  ...
  .req-row.active { border-left:3px solid var(--accent-yellow); background:rgba(255,214,0,.06); }
  .waiting-badge { color:var(--accent-yellow); font-weight:700; }
...
<div id="app">
  ...
  <div class="panel active" id="panel-intercept">
    <div id="intercept-toolbar">
      ...
      <span style="font-size:.72rem; color:var(--text-dim);" id="intercept-scope-summary">No scope set = all traffic</span>
    </div>
    ...
    <div id="editor-area" style="display:none; flex:1; flex-direction:column; min-height:0;">
      <div class="detail-urlrow">
        <span class="method-badge" id="int-method-badge"></span>
        <span class="detail-url-text" id="int-url-text"></span>
        <button class="icon-btn" id="int-close-btn" title="Close" style="display:none;">&#10005;</button>
      </div>
      <div class="detail-tabs" id="int-detail-tabs">
        <button class="detail-tab-btn active" data-idtab="summary">Summary</button>
        <button class="detail-tab-btn" data-idtab="raw">Raw</button>
        <button class="detail-tab-btn" data-idtab="headers">Headers <span id="int-headers-count"></span></button>
        <button class="detail-tab-btn" data-idtab="body">Body</button>
        <button class="detail-tab-btn" data-idtab="cookies">Cookies <span id="int-cookies-count"></span></button>
      </div>
      <div class="detail-content" id="int-detail-content"></div>
      <div class="action-bar" id="int-action-bar">
        <button class="btn-red" id="drop-btn">DROP</button>
        <button class="btn-green" id="forward-btn">FORWARD</button>
        <button class="btn-blue" id="int-send-repeater-btn">TO REPEATER</button>
      </div>
      <div class="detail-reqresp-toggle" id="int-reqresp-toggle" style="display:none;">
        <button data-idside="request">&#8593; Request</button>
        <button class="active" data-idside="response">&#8595; Response</button>
      </div>
    </div>
  </div>
  ...
  <div class="panel" id="panel-settings">
    <div class="settings-scroll">
      ...
      <div class="field-block">
        <label>Methods in scope (none selected = all methods)</label>
...
function renderQueue(items) {
  ...
  items.forEach(item => {
    ...
    row.className = 'req-row' + (item.id === currentFlowId ? ' active' : '');
    row.innerHTML = `
      <div class="row-top"><span class="waiting-badge">&#9203; waiting</span><span class="len">${escapeHtml(item.host)}</span></div>
      <div class="row-url"><span class="method">${item.method}</span>${escapeHtml(item.path || item.url)}</div>
    `;
  ...
  else if (flowsCache[currentFlowId]) renderInterceptDetail();  // keep the open item's data fresh
}

let intResolvedId = null;    // flow id that's been forwarded and is now showing Request/Response like History
let intResolvedSide = 'response';
let intPollTimer = null;

function currentIntSide() { return (currentFlowId === intResolvedId) ? intResolvedSide : 'request'; }

function updateIntCounts(item, side) {
  const headers = side === 'request' ? item.req_headers : item.resp_headers;
  document.getElementById('int-headers-count').innerHTML = `(${Object.keys(headers || {}).length})`;
  const cookies = side === 'request' ? item.req_cookies : item.resp_cookies;
  document.getElementById('int-cookies-count').innerHTML = (cookies || []).length ? `(${cookies.length})` : '';
}

function resetIntToPending() {
  if (intPollTimer) { clearTimeout(intPollTimer); intPollTimer = null; }
  intResolvedId = null;
  document.getElementById('int-action-bar').style.display = 'flex';
  document.getElementById('int-reqresp-toggle').style.display = 'none';
  document.getElementById('int-close-btn').style.display = 'none';
}

function selectFlow(id) {
  if (id !== intResolvedId) resetIntToPending();
  currentFlowId = id;
  const item = flowsCache[id];
  if (!item) return;
  els.editorArea.style.display = 'flex';
  document.getElementById('int-method-badge').textContent = item.method;
  document.getElementById('int-method-badge').className = `method-badge m-${item.method}`;
  document.getElementById('int-url-text').textContent = item.url;
  updateIntCounts(item, currentIntSide());
...
function renderInterceptDetail() {
  ...
  if (!item) return;
  const el = document.getElementById('int-detail-content');
  el.innerHTML = buildTabHtml(intTab, item, currentIntSide());
  wireAccordions(el);
}
document.querySelectorAll('#int-detail-tabs .detail-tab-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    document.querySelectorAll('#int-detail-tabs .detail-tab-btn').forEach(b => b.classList.remove('active'));
    btn.classList.add('active');
    intTab = btn.dataset.idtab;
    renderInterceptDetail();
  });
});
document.querySelectorAll('#int-reqresp-toggle button').forEach(btn => {
  btn.addEventListener('click', () => {
    intResolvedSide = btn.dataset.idside;
    document.querySelectorAll('#int-reqresp-toggle button').forEach(b => b.classList.remove('active'));
    btn.classList.add('active');
    updateIntCounts(flowsCache[currentFlowId], intResolvedSide);
    renderInterceptDetail();
  });
});
document.getElementById('int-close-btn').addEventListener('click', () => {
  const id = currentFlowId;
  resetIntToPending();
  delete flowsCache[id];
  currentFlowId = null;
  els.editorArea.style.display = 'none';
  els.emptyState.style.display = 'flex';
});

async function pollIntResolved(flowId, triesLeft) {
  if (intResolvedId !== flowId) return;   // user moved on to something else -- stop
  try {
    const res = await fetch(`/api/history/${flowId}`, { cache: 'no-store' });
    if (res.ok) {
      const data = await res.json();
      flowsCache[flowId] = data;   // now carries both req_* and resp_* fields, just like a History entry
      if (currentFlowId === flowId) {
        updateIntCounts(data, currentIntSide());
        renderInterceptDetail();
      }
      if (data.status == null && triesLeft > 0) {
        intPollTimer = setTimeout(() => pollIntResolved(flowId, triesLeft - 1), 600);
        return;
      }
    }
...
function showIntResolved(flowId) {
  intResolvedId = flowId;
  intResolvedSide = 'response';
  document.getElementById('int-action-bar').style.display = 'none';
  document.getElementById('int-reqresp-toggle').style.display = 'flex';
  document.getElementById('int-close-btn').style.display = 'inline-block';
  document.querySelectorAll('#int-reqresp-toggle button').forEach(b => b.classList.toggle('active', b.dataset.idside === 'response'));
  updateIntCounts(flowsCache[flowId], 'response');
  renderInterceptDetail();
  pollIntResolved(flowId, 20);   // ~12s of polling for the response to land
}
...
// Forward/Drop send the intercepted request through unedited -- no inline
// editing here anymore. To change something, use "TO REPEATER" instead: it
// opens a fresh, editable copy in the Repeater tab (a brand-new HTTP call,
// same as Burp/Reqable's own Repeater), while this held request is forwarded
// as-is so the app on your phone doesn't hang waiting for a response.
async function sendAction(action) {
  if (!currentFlowId) return null;
  const id = currentFlowId;
  await fetch(`/api/flow/${id}/${action}`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}),
  });
  return id;
}
els.forwardBtn.addEventListener('click', async () => {
  const id = await sendAction('forward');
  if (!id) return;
  showIntResolved(id);   // keep viewing this flow -- now with a Request/Response toggle, like History
});
els.dropBtn.addEventListener('click', async () => {
  const id = await sendAction('drop');
  if (!id) return;
  resetIntToPending();
  delete flowsCache[id];
  currentFlowId = null;
...
document.getElementById('int-send-repeater-btn').addEventListener('click', async () => {
  const item = flowsCache[currentFlowId];
  if (!item) return;
  await sendToRepeater(item.method, item.url, item.req_headers, item.req_body);
  sendAction('forward');
});
...
function wireAccordions(container) {
  container.querySelectorAll('.accordion-head').forEach(h => {
    h.addEventListener('click', () => {
      ...
      const body = container.querySelector(`.accordion-body[data-acc-body="${h.dataset.acc}"]`);
      if (body) body.classList.toggle('open');
    });
  });
}

// Builds the HTML for one detail tab. Shared by History Detail (which has both
// a request and a response side) and Intercept (request side only, no response yet).
function buildTabHtml(tab, d, side) {
  if (tab === 'summary') {
    const conn = d.conn || {};
    ...
    html += `
      ...
      <div class="accordion-body" data-acc-body="app">
      ...
      <div class="accordion-body" data-acc-body="conn">
      ...
      <div class="accordion-body" data-acc-body="tls">
    ...
    return `<pre class="raw-view">${escapeHtml(buildRawText(side, d))}</pre>`;
  } else if (tab === 'headers') {
    ...
    return `<pre class="raw-view">${escapeHtml(body || '(empty body)')}</pre>`;
  } else if (tab === 'cookies') {
...
function renderDetailContent() {
  ...
  el.innerHTML = buildTabHtml(tab, detailData, detailSide);
  wireAccordions(el);
}
...
function updateScopeSummary(s) {
  const parts = [];
  if ((s.domain_include || []).length) parts.push(`only: ${s.domain_include.join(', ')}`);
  if ((s.domain_exclude || []).length) parts.push(`except: ${s.domain_exclude.join(', ')}`);
  if (s.path_regex) parts.push(`path~ ${s.path_regex}`);
  const methodCount = (s.methods || []).length;
  const methodsTxt = (methodCount === 0 || methodCount === ALL_METHODS.length) ? '' : `[${s.methods.join(',')}]`;
  document.getElementById('intercept-scope-summary').textContent =
    parts.length ? `Scope: ${parts.join(' | ')} ${methodsTxt}`.trim() : `No domain/path scope set -- all traffic ${methodsTxt}`.trim();
}
...
// Periodic self-heal: re-reads the real server-side setting every few seconds
// so the pill can never silently drift out of sync with what's actually running
// (e.g. after a flaky mobile-network POST, or a page that's been open a while).
setInterval(loadSettings, 4000);

// The Intercept tab's pill is a one-tap master switch -- flips immediately,
// no need to go into Scope > Save to turn interception on/off. It trusts
// the server's *returned* state rather than assuming the POST succeeded,
// so a dropped request never leaves the UI showing the wrong thing.
document.getElementById('intercept-pill').addEventListener('click', async () => {
  ...
  try {
    const res = await fetch('/api/settings', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ intercept_enabled: wantOn }),
    });
    ...
    alert('Could not reach the proxy to change Intercept -- check your connection and try again.');
    loadSettings();  // re-sync to whatever the server actually has
  } finally {
...
document.getElementById('settings-save').addEventListener('click', async () => {
  ...
  syncInterceptPill(!!s.intercept_enabled);
  updateScopeSummary(s);
  // reflect the server's sanitized domain strings back into the fields (scheme/path stripped)
  document.getElementById('set-domain-include').value = (s.domain_include || []).join(', ');
  document.getElementById('set-domain-exclude').value = (s.domain_exclude || []).join(', ');
